...
async function main() {
  ...
  const JMACreativeNFTContractFactory = await ethers.getContractFactory(
    "JMACreativeNFTContract"
  );
  // deploy
  const JMACreativeNFTContract = await JMACreativeNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await JMACreativeNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await JMACreativeNFTContract.mintTo(ipfs[i], {
